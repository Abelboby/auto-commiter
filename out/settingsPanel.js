...
async function openSettingsPanel(context) {
    ...
    const render = async () => {
        ...
        const codiconCssUri = panel.webview.asWebviewUri(vscode.Uri.joinPath(context.extensionUri, "media", "codicons", "codicon.css"));
...
async function verifyGroqApiKey(apiKey) {
    if (!apiKey) {
        return { ok: false, message: "Enter a Groq API key first." };
    }
    return new Promise((resolve) => {
        const request = https.request("https://api.groq.com/openai/v1/models", {
            method: "GET",
            headers: {
